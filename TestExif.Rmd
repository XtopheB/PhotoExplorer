...
```{r paths}
# Source directory
SourceDir <-  "c:/temp/Photos/Iphone/202104__"
...
### Where the `r nrow(mydat)` pictures will be copied? 
...
NamesOriginal<- word( gsub("/", " ", Myfiles), -1)
NameFinal <- word( gsub("/", " ", MyCopiedfiles), -1)

NbCommon <- length(intersect(NameFinal, NamesOriginal))

message <- ifelse( NbCommon == nrow(mydat),
                   paste("Everything went well (", length(NameFinal) , "files copied)"),
