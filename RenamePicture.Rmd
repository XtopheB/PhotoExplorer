...
mydatN <- mydatN %>%
  group_by(Country, Year, Newcity) %>%
  mutate( NbPicturesCity = n_distinct(Date))
...
mydatN <- mydatN %>%
   mutate(SubDir = case_when(NbPicturesDay > NbPicturesDayMax ~ paste0(Month,"_", Newcity),
                          NbPicturesCity > NbPicturesCityMax ~ paste0(Month,"_", Newcity),
                             .default =  SubDir)
   )
